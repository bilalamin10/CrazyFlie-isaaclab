...
df = pd.read_csv("logs/success_thresholds.csv")
shapes = ["Hover", "Static", "Circle", "Lemniscate", "Lissajous"]

# Success rate heatmap
mean = df.groupby(["train_shape", "eval_shape"])["success_50cm"].mean().unstack()
std = df.groupby(["train_shape", "eval_shape"])["success_50cm"].std().unstack()
...
ax.set_title("Cross-trajectory generalization: mean success_rate_50 (m)\nlower is better, mean ± std over 3 seeds")
plt.colorbar(im, ax=ax, label="Mean success_rate_50 (m)")
plt.tight_layout()
