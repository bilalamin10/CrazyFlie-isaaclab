...
mean = df.groupby(["train_shape", "eval_shape"])["tracking_err_mean"].mean().unstack()
std = df.groupby(["train_shape", "eval_shape"])["tracking_err_mean"].std().unstack()
mean = mean.reindex(index=shapes, columns=shapes)
std = std.reindex(index=shapes, columns=shapes)

fig, ax = plt.subplots(figsize=(8, 7))
im = ax.imshow(mean.values, cmap="RdYlGn_r", vmin=0, vmax=mean.values.max())
...
ax.set_title("Cross-trajectory generalization: mean tracking error (m)\nlower is better, mean ± std over 3 seeds")
plt.colorbar(im, ax=ax, label="Mean tracking error (m)")
plt.tight_layout()
plt.savefig("logs/tracking_error_matrix.png", dpi=150, bbox_inches="tight")
print("Saved logs/tracking_error_matrix.png")
plt.show()